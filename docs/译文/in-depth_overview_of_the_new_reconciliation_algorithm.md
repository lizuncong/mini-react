...
React 中的每个组件都有一个 UI 表示，我们可以调用从 render 方法返回的视图或模板。这是我们 ClickCounter 组件的模板：
...
一旦模板通过 JSX 编译器，您最终会得到一堆 React 元素。这是真正从 renderReact 组件的方法返回的，而不是 HTML。由于我们不需要使用 JSX，render 我们的 ClickCounter 组件的方法可以这样重写：
...
React.createElement 方法中的调用 render 将创建两个数据结构，如下所示：
...
您可以看到 React 将属性添加$$typeof 到这些对象以将它们唯一标识为 React 元素。然后我们有属性 type，它描述了元素。这些值取自您传递给函数的值。注意 React 如何将文本内容表示为和节点的子节点。以及点击处理程序如何成为元素道具的一部分。React 元素上还有其他字段，例如超出本文范围的字段。keypropsReact.createElementspanbuttonbuttonref
